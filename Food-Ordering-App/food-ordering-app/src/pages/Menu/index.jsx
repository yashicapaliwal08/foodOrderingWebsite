...
const Menu=()=>{
    ...
    const [activeTab,setActiveTab]=useState('');//setActiveTab to update the state
    ...
    // in menu changes change in menu is happening due to this
    const onTabSwitch=(newActiveTab)=>{
        setActiveTab(newActiveTab);
        let categories=products.products.map((product)=>product.name.name);
        let index=categories.findIndex(category=>newActiveTab===category);// matching the name of category with active tab
        console.log(index);
        if(index>-1){
