...
const Login=()=>{
    ...
    const onSubmit=(data)=>{
        ...
        let uid='';
        signInWithEmailAndPassword(authentication,data.email,data.password).then((response)=>{
            uid=response.user.uid;
            sessionStorage.setItem('User Id',uid);
            sessionStorage.setItem('Auth token',response._tokenResponse.refreshToken)
            //to get listeners updated 
    ...
    return (
        <div className="h-screen bg-red-500 flex items-center justify-center">
            <div className="rounded-lg max-w-md w-full flex flex-col items-center justify-center relative">
                <div className="absolute inset-0 transition duration-300 animate-pink blur gradient bg-gradient-to-tr from-rose-400 to-yellow-200"></div>
                <div className="p-10 rounded-xl z-10 w-full h-full bg-red-500">
